...
class CalendarViewHolder constructor(private val binding: CalendarHeaderBinding) :
...
class CalendarAdapter(
    private var calArray: ArrayList<Calendar>,
    var click: (ArrayList<MemoData>?, Calendar,Int) -> (Unit)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == HEADER) {
            var binding = DataBindingUtil.inflate<CalendarHeaderBinding>(
            ...
            var binding = DataBindingUtil.inflate<EmptyDayBinding>(
                LayoutInflater.from(parent.context),
    ...
    var touchTwice:Any?=null
    var priviousHolder:LinearLayout?=null
        ...
        when (type) {
            ...
            DAY -> {
                ...
                if (find >= 0) {
                    ...
                    val layoutManager = LinearLayoutManager(CalendarActivity.context)
                    layoutManager.orientation = LinearLayoutManager.VERTICAL
                    holder.itemView.dailyRecyclerView.layoutManager = layoutManager
                    holder.itemView.dailyRecyclerView.adapter = DailyListAdapter(temp)
                    holder.itemView.calendarLayout.setOnClickListener {
                        if(touchTwice!=item){
                            touchTwice=item
                            holder.itemView.calendarLayout.setBackgroundResource(R.drawable.calendar_day_pressed_event)
                            if(priviousHolder!=null){
                                priviousHolder?.setBackgroundResource(R.drawable.calendar_day_nopressed_event)
                            }
                            priviousHolder=holder.itemView.calendarLayout
                        }
                        else {
                            holder.itemView.calendarLayout.setBackgroundResource(R.drawable.calendar_day_nopressed_event)
                            click(temp, model.mCalendar.value!!,position)
                        }
                    }
                } else {
                    holder.itemView.calendarLayout.setOnClickListener {
                        ...
                        else{holder.itemView.calendarLayout.setBackgroundResource(R.drawable.calendar_day_nopressed_event)
                            click(null, model.mCalendar.value!!,position)
                        }
                    }
                }
                holder.setViewModel(model)
    ...
    private fun findMemo(item: Calendar): ArrayList<MemoData> {
        ...
        for (i in CalendarActivity.memoData) {
            if (i.day == item.get(Calendar.DATE) && i.month == item.get(Calendar.MONTH) && i.year == item.get(
                    Calendar.YEAR
                )
            ) {
                tempArray.add(i)
