...
class ChooseTransferList : AppCompatActivity() {
    ...
    lateinit var urlName:String
    lateinit var urlParsing:Uri
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val startLocation = intent.getParcelableExtra<LatLng>("start")
        val endLocation = intent.getParcelableExtra<LatLng>("destination")
        val startName="   ${DirectionFinder().convertToAddress(
            startLocation.latitude,
            startLocation.longitude
        )}"
        val endName= "${DirectionFinder().convertToAddress(endLocation.latitude, endLocation.longitude)}"
        StartText.text = startName
        endText.text =endName
        urlName="https://www.google.com/maps/dir/${startLocation.latitude},${startLocation.longitude}/${endLocation.latitude},${endLocation.longitude}?hl=ko"
//        var url = DirectionFinder().getPublicDataUrl(startLocation, endLocation)
//        UsingPublicAPI(url).execute()
        urlParsing= Uri.parse(urlName)
        transferWebView.loadUrl(urlName)
        transferWebView.settings.javaScriptEnabled=true
        transferWebView.webChromeClient= WebChromeClient()
        transferWebView.webViewClient= WebViewClientClass()
    }
    ...
    override fun onBackPressed() {
        ...
        setResult(5, Intent(this,MainActivity::class.java))
        finish()
    }
